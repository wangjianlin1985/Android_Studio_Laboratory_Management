...
public class TeacherDAO {
    ...
    public ArrayList<Teacher> QueryTeacherInfo(String teacherNo,String name,String inDate,String telephone,int currentPage) { 
    	Session s = factory.getCurrentSession();
    	...
    	if(!inDate.equals("")) hql = hql + " and teacher.inDate like '%" + inDate + "%'";
    ...
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public ArrayList<Teacher> QueryTeacherInfo(String teacherNo,String name,String inDate,String telephone) { 
    	Session s = factory.getCurrentSession();
    	String hql = "From Teacher teacher where 1=1";
    	if(!teacherNo.equals("")) hql = hql + " and teacher.teacherNo like '%" + teacherNo + "%'";
    	if(!name.equals("")) hql = hql + " and teacher.name like '%" + name + "%'";
    ...
    /*计算总的页数和记录数*/
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public void CalculateTotalPageAndRecordNumber(String teacherNo,String name,String inDate,String telephone) {
        Session s = factory.getCurrentSession();
        String hql = "From Teacher teacher where 1=1";
        if(!teacherNo.equals("")) hql = hql + " and teacher.teacherNo like '%" + teacherNo + "%'";
        if(!name.equals("")) hql = hql + " and teacher.name like '%" + name + "%'";
