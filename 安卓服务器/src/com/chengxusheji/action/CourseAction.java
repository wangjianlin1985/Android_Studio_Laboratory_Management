...
public class CourseAction extends BaseAction {
    ...
    public String AddCourse() {
        ...
        try {
            ClassInfo classObj = classInfoDAO.GetClassInfoByClassNo(course.getClassObj().getClassNo());
            course.setClassObj(classObj);
    ...
    public String QueryCourse() {
        ...
        List<Course> courseList = courseDAO.QueryCourseInfo(courseNo, classObj, courseName, teacherObj, currentPage);
        /*计算总的页数和总的记录数*/
        courseDAO.CalculateTotalPageAndRecordNumber(courseNo, classObj, courseName, teacherObj);
        /*获取到总的页码数目*/
    ...
    public String QueryCourseOutputToExcel() { 
        if(courseNo == null) courseNo = "";
        if(courseName == null) courseName = "";
        List<Course> courseList = courseDAO.QueryCourseInfo(courseNo,classObj,courseName,teacherObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Course信息记录"; 
        String[] headers = { "课程编号","所属班级","课程名称","课程总学时","课程学分","上课老师","课程描述"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<courseList.size();i++) {
        	Course course = courseList.get(i); 
        	dataset.add(new String[]{course.getCourseNo(),course.getClassObj().getClassName(),
course.getCourseName(),course.getCourseHours() + "",course.getCourseScore() + "",course.getTeacherObj().getName(),
