...
public class StudentAction extends BaseAction {
    ...
    public String QueryStudent() {
        ...
        List<Student> studentList = studentDAO.QueryStudentInfo(studentNo, classObj, name, birthDate, telephone, currentPage);
        /*计算总的页数和总的记录数*/
        studentDAO.CalculateTotalPageAndRecordNumber(studentNo, classObj, name, birthDate, telephone);
        /*获取到总的页码数目*/
    ...
    public String QueryStudentOutputToExcel() { 
        ...
        if(birthDate == null) birthDate = "";
        if(telephone == null) telephone = "";
        List<Student> studentList = studentDAO.QueryStudentInfo(studentNo,classObj,name,birthDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Student信息记录"; 
        String[] headers = { "学号","所在班级","姓名","性别","出生日期","学生照片","联系电话","邮箱"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<studentList.size();i++) {
        	Student student = studentList.get(i); 
        	dataset.add(new String[]{student.getStudentNo(),student.getClassObj().getClassName(),
student.getName(),student.getGender(),new SimpleDateFormat("yyyy-MM-dd").format(student.getBirthDate()),student.getStudentPhoto(),student.getTelephone(),student.getEmail()});
        }
        /*
