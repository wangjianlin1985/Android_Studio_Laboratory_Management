...
public class TeacherAction extends BaseAction {
    ...
    public String QueryTeacher() {
        ...
        List<Teacher> teacherList = teacherDAO.QueryTeacherInfo(teacherNo, name, inDate, telephone, currentPage);
        /*计算总的页数和总的记录数*/
        teacherDAO.CalculateTotalPageAndRecordNumber(teacherNo, name, inDate, telephone);
        /*获取到总的页码数目*/
    ...
    public String QueryTeacherOutputToExcel() { 
        ...
        if(inDate == null) inDate = "";
        if(telephone == null) telephone = "";
        List<Teacher> teacherList = teacherDAO.QueryTeacherInfo(teacherNo,name,inDate,telephone);
        ExportExcelUtil ex = new ExportExcelUtil();
        String title = "Teacher信息记录"; 
        String[] headers = { "教师编号","姓名","性别","入职日期","教师照片","联系电话"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<teacherList.size();i++) {
        	Teacher teacher = teacherList.get(i); 
        	dataset.add(new String[]{teacher.getTeacherNo(),teacher.getName(),teacher.getSex(),new SimpleDateFormat("yyyy-MM-dd").format(teacher.getInDate()),teacher.getTeacherPhoto(),teacher.getTelephone()});
        }
        /*
