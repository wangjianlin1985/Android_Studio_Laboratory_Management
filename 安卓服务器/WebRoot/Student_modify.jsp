...
<table width='100%' cellspacing='1' cellpadding='3' class="tablewidth">
  ...
  <tr>
    <td width=30%>所在班级:</td>
    <td width=70%>
      <select name="student.classObj.classNo">
      <%
        for(ClassInfo classInfo:classInfoList) {
          String selected = "";
          if(classInfo.getClassNo().equals(student.getClassObj().getClassNo()))
            selected = "selected";
      %>
  ...
  <tr>
    <td width=30%>性别:</td>
    <td width=70%><input id="student.gender" name="student.gender" type="text" size="4" value='<%=student.getGender() %>'/></td>
  </tr>

  <tr>
    <td width=30%>出生日期:</td>
    <% DateFormat birthDateSDF = new SimpleDateFormat("yyyy-MM-dd");  %>
    <td width=70%><input type="text" readonly  id="student.birthDate"  name="student.birthDate" onclick="setDay(this);" value='<%=birthDateSDF.format(student.getBirthDate()) %>'/></td>
  </tr>
  ...
  <tr>
    <td width=30%>邮箱:</td>
    <td width=70%><input id="student.email" name="student.email" type="text" size="50" value='<%=student.getEmail() %>'/></td>
  </tr>
