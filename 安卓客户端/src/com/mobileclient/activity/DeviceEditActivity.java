			...
			public void onClick(View v) {
				try {
					...
					if(ET_deviceName.getText().toString().equals("")) {
						Toast.makeText(DeviceEditActivity.this, "设备名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if (!device.getDevicePhoto().startsWith("upload/")) {
						...
						DeviceEditActivity.this.setTitle("正在上传图片，稍等...");
						String devicePhoto = HttpUtil.uploadFile(device.getDevicePhoto());
						DeviceEditActivity.this.setTitle("图片上传完毕！");
						device.setDevicePhoto(devicePhoto);
					} 
					/*验证获取设备价格*/ 
					if(ET_devicePrice.getText().toString().equals("")) {
						Toast.makeText(DeviceEditActivity.this, "设备价格输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_deviceCount.getText().toString().equals("")) {
						Toast.makeText(DeviceEditActivity.this, "设备数量输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_deviceDesc.getText().toString().equals("")) {
						Toast.makeText(DeviceEditActivity.this, "设备描述输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传实验设备信息*/
					DeviceEditActivity.this.setTitle("正在更新实验设备信息，稍等...");
