			...
			public void onClick(View v) {
				try {
					...
					if(ET_testName.getText().toString().equals("")) {
						Toast.makeText(CourseTestEditActivity.this, "实验名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_labTime.getText().toString().equals("")) {
						Toast.makeText(CourseTestEditActivity.this, "实验时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_testMemo.getText().toString().equals("")) {
						Toast.makeText(CourseTestEditActivity.this, "实验备注输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传课程实验信息*/
					CourseTestEditActivity.this.setTitle("正在更新课程实验信息，稍等...");
