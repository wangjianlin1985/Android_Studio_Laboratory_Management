...
public class CourseTestAddActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.coursetest_add); 
			...
			public void onClick(View v) {
				try {
					...
					if(ET_testName.getText().toString().equals("")) {
						Toast.makeText(CourseTestAddActivity.this, "实验名称输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_labTime.getText().toString().equals("")) {
						Toast.makeText(CourseTestAddActivity.this, "实验时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_testMemo.getText().toString().equals("")) {
						Toast.makeText(CourseTestAddActivity.this, "实验备注输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传课程实验信息*/
					CourseTestAddActivity.this.setTitle("正在上传课程实验信息，稍等...");
