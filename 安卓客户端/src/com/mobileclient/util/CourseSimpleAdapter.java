...
/*需要绑定的数据*/
  ...
  public View getView(int position, View convertView, ViewGroup parent) { 
	  ...
	  /*设置各个控件的展示内容*/
	  holder.tv_courseNo.setText("课程编号：" + mData.get(position).get("courseNo").toString());
	  holder.tv_classObj.setText("所属班级：" + (new ClassInfoService()).GetClassInfo(mData.get(position).get("classObj").toString()).getClassName());
	  holder.tv_courseName.setText("课程名称：" + mData.get(position).get("courseName").toString());
	  holder.tv_courseHours.setText("课程总学时：" + mData.get(position).get("courseHours").toString());
	  holder.tv_courseScore.setText("课程学分：" + mData.get(position).get("courseScore").toString());
	  holder.tv_teacherObj.setText("上课老师：" + (new TeacherService()).GetTeacher(mData.get(position).get("teacherObj").toString()).getName());
	  holder.tv_courseDesc.setText("课程描述：" + mData.get(position).get("courseDesc").toString());
	  /*返回修改好的view*/
	  return convertView; 
    } 
